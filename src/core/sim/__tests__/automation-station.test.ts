...
import {
  ...
  automationInputs,
  parseWorkpieceQueue,
  stepAutomationStation,
  AUTOMATION_MAGAZINE_MAX,
  type AutomationOutputChannel,
  type AutomationStationState,
} from "../automation-station";

/**
 * Phase 14-2: 자동화설비 스테이션 물리 상태기계 단위 테스트.
...
function run(
  ...
  on: AutomationOutputChannel[],
  seconds: number,
): void {
  const set = new Set(on);
  const steps = Math.round(seconds / DT);
  for (let i = 0; i < steps; i++) stepAutomationStation(state, (ch) => set.has(ch), DT);
}
...
describe("매거진 큐 파싱", () => {
  ...
  it("최대 적재 수를 넘지 않는다", () => {
    ...
    expect(parseWorkpieceQueue(many)).toHaveLength(AUTOMATION_MAGAZINE_MAX);
  });
...
describe("A실린더 공급 + 판별 센서", () => {
  it("A 전진 완료 시 매거진 → 공급 위치 (판별 센서는 아직 반응 없음 — 벨트 초입 배치)", () => {
    const state = createAutomationState({ workpieces: "금,비" });
    expect(automationInputs(state).매거진).toBe(true);

    run(state, ["A전솔"], 0.7); // 전 행정 0.5s + 여유
    expect(automationInputs(state).A전센).toBe(true);
    expect(state.supply).toBe("metal");
    expect(state.magazine).toHaveLength(1);
    // 배치도(S3/S4): 판별 센서는 컨베이어 초입에 있어 공급 위치에서는 감지하지 않는다
    expect(automationInputs(state).용량형).toBe(false);
    expect(automationInputs(state).유도형).toBe(false);

    run(state, ["A후솔"], 0.7); // 양솔 복귀
    expect(automationInputs(state).A후센).toBe(true);
  });

  it("판별은 벨트 초입 구간에서: 금속이면 용량형+유도형, 비금속이면 용량형만", () => {
    const metal = createAutomationState({ workpieces: "금" });
    run(metal, ["A전솔"], 0.7);
    run(metal, ["A후솔"], 0.7);
    run(metal, ["C전솔"], 0.7); // 벨트 초입 이송
    run(metal, ["컨베이어"], 0.8); // 감지 구간(0.06~0.24) 진입
    expect(automationInputs(metal).용량형).toBe(true);
    expect(automationInputs(metal).유도형).toBe(true);

    const nonmetal = createAutomationState({ workpieces: "비" });
    run(nonmetal, ["A전솔"], 0.7);
    run(nonmetal, ["A후솔"], 0.7);
    run(nonmetal, ["C전솔"], 0.7);
    run(nonmetal, ["컨베이어"], 0.8);
    expect(automationInputs(nonmetal).용량형).toBe(true);
    expect(automationInputs(nonmetal).유도형).toBe(false); // 유도형은 금속만
  });

  it("양솔 임펄스: 전진 1스캔 펄스 후 무신호여도 전진 종단까지 간다 (스풀 메모리)", () => {
    const state = createAutomationState({});
    stepAutomationStation(state, (ch) => ch === "A전솔", DT); // 1스캔 펄스만
    run(state, [], 1); // 이후 무신호
  ...
  it("양솔 임펄스: 후진 1스캔 펄스 후 무신호여도 후진 종단까지 온다", () => {
    ...
    stepAutomationStation(state, (ch) => ch === "A후솔", DT); // 후진 1스캔 펄스
    run(state, [], 1);
  ...
  it("양솔 임펄스: 양측 동시 여자 시 기존 스풀 방향을 유지한다", () => {
    const state = createAutomationState({});
    stepAutomationStation(state, (ch) => ch === "A전솔", DT); // 전진 스풀
  ...
  it("양솔 임펄스: 이동 중 반대 펄스가 오면 방향을 전환한다", () => {
    const state = createAutomationState({});
    stepAutomationStation(state, (ch) => ch === "A전솔", DT);
    run(state, [], 0.3); // 전진 중
    const mid = state.cyl.A;
    expect(mid).toBeGreaterThan(0.3);
    expect(mid).toBeLessThan(1);
    stepAutomationStation(state, (ch) => ch === "A후솔", DT); // 반대 펄스
    run(state, [], 0.5);
  ...
  it("빈 매거진: 센서 꺼짐, 전진해도 무변화", () => {
    const state = createAutomationState({ workpieces: "금" });
    run(state, ["A전솔"], 0.7);
    run(state, ["A후솔"], 0.7);
    expect(automationInputs(state).매거진).toBe(false);
    run(state, ["A전솔"], 0.7);
...
describe("편솔 스프링 복귀 (B/C/D)", () => {
  it("전솔 OFF면 후진한다", () => {
    const state = createAutomationState({});
    run(state, ["B전솔"], 0.7);
    expect(automationInputs(state).B전센).toBe(true);
    run(state, [], 0.7);
    expect(automationInputs(state).B후센).toBe(true);
  });
});

describe("C 이송 → 컨베이어 → 분류", () => {
  /** 물품 하나를 공급 위치까지 올려놓는 헬퍼 */
  function supplied(material: string): AutomationStationState {
    const state = createAutomationState({ workpieces: material });
    run(state, ["A전솔"], 0.7);
    run(state, ["A후솔"], 0.7);
    return state;
  }
  ...
  it("비금속: D 미작동 → 포토센서 통과 → 컨베이어 끝 저장박스", () => {
    ...
    expect(automationInputs(state).포토).toBe(true);
...
describe("Phase 14-3: 엔진 ioMap 채널 연동 (PLC ↔ 스테이션)", () => {
  it("PB1 입력 채널 → 래더 → 녹램 출력 채널로 전파된다 (출력 반영 후 다음 틱에 물리 관찰)", async () => {
    ...
    expect((snap.components[stationId].equipment as AutomationStationState | undefined)?.lamps.green).toBe(false);
    const offset1 = (snap.components[stationId].equipment as AutomationStationState | undefined)?.beltOffset ?? 0;
    ...
    engine.tick(0.02);
    snap = engine.snapshot();
    expect((snap.components[stationId].equipment as AutomationStationState | undefined)?.lamps.green).toBe(true);
...
describe("골든 시나리오: MPS 자동운전 (예제 19 — 수업자료 슬라이드 9 LD)", () => {
  ...
  async function runExample(workpieces: string, seconds: number, pb3At?: number) {
    ...
    const { SimulationEngine } = await import("../engine");
    const doc = getExample("automation-basic")!.build();
    const station = doc.components.find((c) => c.type === "auto.automation-station")!;
    ...
    return { mps: engine.snapshot().components[station.id].equipment as AutomationStationState, engine, stationId: station.id };
  }
  ...
  it("PB4 일시정지: 누르면 출력 차단·적램 점멸, 떼면(음변환) 초기화된다", async () => {
    ...
    let mps = engine.snapshot().components[station.id].equipment as AutomationStationState;
    expect(mps.lamps.red).toBe(true); // 적램 (점멸 위상 초반 ON)
    const pausedB = mps.cyl.B;

    for (let i = 0; i < 25; i++) engine.tick(0.02); // 0.5s 더 정지 유지
    mps = engine.snapshot().components[station.id].equipment as AutomationStationState;
...
describe("P1-6: 다채널 capability 일반화 정합성", () => {
  it("정의 메타데이터(ioChannels)와 상태기계 채널 목록이 일치한다", async () => {
    ...
    const { AUTOMATION_INPUT_CHANNELS, AUTOMATION_OUTPUT_CHANNELS } = await import("../automation-station");
    const def = getComponentDefinition("auto.automation-station");
    ...
    expect(inChans).toEqual([...AUTOMATION_INPUT_CHANNELS]);
    expect(outChans).toEqual([...AUTOMATION_OUTPUT_CHANNELS]);
  });

  it("복합설비 어댑터가 부품 type으로 등록·조회된다", async () => {
    const { getEquipmentAdapter } = await import("../equipment-adapter");
    await import("../automation-station"); // 등록 side-effect
    const adapter = getEquipmentAdapter("auto.automation-station");
