...
export const examples: ExampleEntry[] = [
  ...
  },
  {
    ...
    build: () =>
      buildCircuit(
        "MPS 자동운전",
        "수업자료 슬라이드 9의 래더를 그대로 재현: PB2 기동 → A공급 → 판별 → B드릴 → C이송 → 컨베이어 → 분류(기본: 금속=D배출박스/비금속=저장박스, PB3 누르면 반전). 사이클 후 매거진에 물품이 있으면 자동 재시작(M22), 없으면 종료(M23). 금1·비1 처리 시 카운터 종료(M25). PB4 누르면 일시정지(적램 점멸)·떼면 초기화(음변환). 녹램=운전 점멸, 황램=판별 점멸. (실기 T0019=3.5s는 시뮬레이터 컨베이어 속도에 맞춰 5.1s)",
        (b) => {
          ...
          b.setPlc(
            {
              rungs: [
                ...
                rungOf(
                  [
                    [lc("no", "M00100"), lc("nc", "M00024"), lc("ton", "T0019", 5.1)],
