...
/**
 * 리밋 스위치 — **사다리 회로 표기** (Phase 19-2, 소유자 제공 실기 도면 기준).
 *
 * 실기 도면은 리밋 스위치를 두 가지로 나눠 그린다. 배치도에는 스위치 몸체(박스 +
 * 스프링 + 플런저 + 롤러)를 그리고, **전기 회로도에는 조작부만 간략히** —
 * 롤러(⊙) + 기계적 연결 파선 + 접점 — 으로 그린다. 사다리에 몸체 박스를 넣으면
 * 회로가 뭉개지므로 여기서는 도면대로 조작부 표기만 쓴다.
 * 몸체 그림은 장비 뷰의 LimitSwitchSprite가 담당한다.
 *
 * 감지 방향은 도면처럼 화살표로 표시한다 — 전진단 ↑ / 후진단 ↓.
 */
function ElecLimitSwitch({ properties, runtime }: SymbolProps): ReactElement {
  const closed = contactClosedNow(properties, runtime);
  const isNC = properties.contactType === "NC";
  const atRetracted = properties.triggerAt === "retracted";
  // 캠이 닿아 눌린 상태 (동작 중에만 표시)
  ...
  return (
    <g>
      ...
      <circle cx={-19} cy={0} r={4.5} {...Sthin} fill="none" />
      <circle cx={-19} cy={0} r={1.4} fill="currentColor" stroke="none" />
      {/* 감지 방향 화살표 — 전진단 ↑ / 후진단 ↓ */}
      <g stroke="currentColor" strokeWidth={1.5} fill="none">
        {atRetracted ? (
          <>
            <line x1={-29} y1={-7} x2={-29} y2={7} />
            <polyline points="-32.5,2.5 -29,7 -25.5,2.5" />
          </>
        ) : (
          <>
            <line x1={-29} y1={7} x2={-29} y2={-7} />
