...
- **압력 조작 밸브**(`pressure-pilot-valve` — 시퀀스·카운터밸런스, Phase 15): 파일럿 포트의 레벨이 설정압 이상이면 portIn→portOut을 개방하고, 파일럿이 무압이 될 때까지 개방을 유지한다(히스테리시스 — 부하압 캡과의 개폐 채터 방지). `checkBypass`는 역방향 자유 흐름(체크 내장)을 뜻한다. 개폐 상태는 릴리프의 `reliefActive`처럼 솔버가 판정해 런타임·스냅숏에 싣는다.
- **부하압 캡**(Phase 15): 실린더 속성 `loadPressure`가 0보다 크고 그 실린더가 실제로 움직일 수 있으면(공급/배기 조합이 성립하고 그 방향으로 행정이 남아 있으면), 공급 포트가 속한 유로 영역 전체의 레벨을 부하압으로 제한한다 — 릴리프의 영역 cap과 같은 메커니즘. 행정 끝에 닿거나 반대편이 갇히면 캡이 사라져 소스 압력까지 상승한다. 기본값 0이면 비활성이라 기존 회로 동작은 그대로다.
- **어큐뮬레이터**(Phase 15): 잔량이 있으면 `충전압 × 잔량` 레벨의 보조 압력원으로 참여한다. 충전/방전 판정은 어큐뮬레이터를 제외한 실제 압력원만으로 공급을 한 번 더 전파해(`accumulatorSupplied`) 외부 공급 유무로 내리고, 방전 적분은 엔진이 `holdTime` 기준으로 수행한다.
